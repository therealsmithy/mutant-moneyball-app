...
lapply(1:nrow(name_link_df), function(x) {
  tryCatch({
    page <- read_html(name_link_df$link[x])
    
    picture_link <- page %>%
      html_elements("div.disambiguation_main_version_image img") %>% 
      html_attr("data-src")
    ...
    if (length(picture_link) == 0) {
      ...
      download.file(
        picture_link, 
        destfile = paste0("images/", name_link_df$alias[x], ".jpg"), 
        headers = c(
    ...
    name_link_df$error[x] <- paste("Error: ", e$message)
  })
})
